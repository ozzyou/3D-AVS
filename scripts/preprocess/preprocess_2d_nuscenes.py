...
def process_one_sequence(scene):
    ...
    timestamp = sorted(os.listdir(os.path.join(data_path, scene, "frames")))[
        -1
    ]  # take only the last timestamp
    for cam in cam_locs:
        img_name = os.path.join(
            data_path, scene, "frames", timestamp, cam, "color_image.jpg"
        )
        ...
        pose_dir = os.path.join(
            data_path, scene, "frames", timestamp, cam, "cam2scene.txt"
        )
        pose = np.asarray(
            [
                [float(x[0]), float(x[1]), float(x[2]), float(x[3])]
                for x in (x.split(" ") for x in open(pose_dir).read().splitlines())
            ]
        )
        ...
        K = np.asarray(
            [
                ...
                for x in (x.split(" ") for x in open(K_dir).read().splitlines())
            ]
        )
