...
parser.add_argument(
    "--export_label_images", dest="export_label_images", action="store_true"
)
parser.add_argument(
    "--label_type", default="label-filt", help="which labels (label or label-filt)"
)
...
parser.add_argument(
    "--output_image_width", type=int, default=320, help="export image width"
)
parser.add_argument(
    "--output_image_height", type=int, default=240, help="export image height"
)
...
def main():
    ...
    if opt.export_label_images:
        label_map = read_label_mapping(
            opt.label_map_file, label_from="id", label_to="nyu40id"
        )
    ...
    scenes = sorted(
        [
            d
            for d in os.listdir(opt.scannet_path)
    ...
    for i in range(0, len(scenes)):
        ...
        if os.path.exists(os.path.join(opt.output_path, scenes[i])) and os.path.exists(
            output_color_path + "/0.jpg"
        ):
        ...
        # read and export
        sys.stdout.write(
            "\r[ %d | %d ] %s\tloading..." % ((i + 1), len(scenes), scenes[i])
        )
        sys.stdout.flush()
        if os.path.exists(sens_file):
            ...
            os.system(
                "python ~/disk2/download-scannet.py -o ~/disk2/scannet --type .sens --id "
                + scenes[i]
            )
            sd = SensorData(sens_file)
        sys.stdout.write(
            "\r[ %d | %d ] %s\texporting..." % ((i + 1), len(scenes), scenes[i])
        )
        sys.stdout.flush()
        ...
        if opt.export_label_images:
            ...
            for f in range(0, len(sd.frames), opt.frame_skip):
                ...
                image = sktf.resize(
                    image,
                    [opt.output_image_height, opt.output_image_width],
                    order=0,
                    preserve_range=True,
                )
                mapped_image = map_label_image(image, label_map)
                imageio.imwrite(
                    os.path.join(output_label_path, str(f) + ".png"), mapped_image
                )
    print("")
