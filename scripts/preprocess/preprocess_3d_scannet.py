...
def process_one_scene(fn):
    ...
    torch.save(
        (coords, colors, w), os.path.join(out_dir, fn[:-4].split("/")[-1] + ".pth")
    )
    print(fn, fn2)
...
for scene in scene_list:
    ...
    files.append(glob.glob(os.path.join(in_path, scene, "*_vh_clean_2.ply"))[0])
    files2.append(glob.glob(os.path.join(in_path, scene, "*_vh_clean_2.labels.ply"))[0])
