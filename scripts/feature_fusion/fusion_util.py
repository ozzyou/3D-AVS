...
def extract_openseg_img_feature(
    img_dir, openseg_model, text_emb, img_size=None, regional_pool=True
):
...
def save_fused_feature(feat_bank, point_ids, n_points, out_dir, scene_id, args):
    ...
    for n in range(args.num_rand_file_per_scene):
        if n_points < args.n_split_points:
            n_points_cur = (
                n_points  # to handle point cloud numbers less than n_split_points
            )
        else:
        ...
        torch.save(
            {"feat": feat_bank[mask_entire].half().cpu(), "mask_full": mask_entire},
