...
def get_args():
    ...
    parser.add_argument(
        "--split", type=str, default="test", help='split: "train"| "val" | "test" '
    )
    parser.add_argument(
        "--openseg_model",
        type=str,
...
def process_one_scene(data_path, out_dir, args):
    ...
    if keep_features_in_memory and openseg_model is not None:
        ...
        for img_dir in tqdm(img_dirs):
            img_features.append(
                extract_openseg_img_feature(img_dir, openseg_model, text_emb)
            )
    ...
    for img_id, img_dir in enumerate(tqdm(img_dirs)):
        ...
        if keep_features_in_memory:
            ...
            feat_2d = extract_openseg_img_feature(img_dir, openseg_model, text_emb).to(
                device
            )
...
def main(args):
    ...
    args.keep_features_in_memory = (
        False  # keep image features in the memory, very expensive
    )
