...
def get_args():
    ...
    parser.add_argument(
        "--data_dir", type=str, help="Where is the base logging directory"
    )
    parser.add_argument(
        "--output_dir", type=str, help="Where is the base logging directory"
    )
    parser.add_argument(
        "--split", type=str, default="test", help='split: "train"| "val" '
    )
    parser.add_argument(
        "--openseg_model",
        type=str,
    ...
    parser.add_argument(
        "--process_id_range", nargs="+", default=None, help="the id range to process"
    )
    parser.add_argument(
        "--img_feat_dir", type=str, default="", help="the id range to process"
    )
...
def process_one_scene(data_path, out_dir, args):
    ...
    for img_id, cam in enumerate(tqdm(cam_locs)):
        ...
        mapping[:, 1:4] = point2img_mapper.compute_mapping(
            pose, locs_in, depth=None, intrinsic=intr
        )
        ...
        feat_2d = extract_openseg_img_feature(
            img_dir, openseg_model, text_emb, img_size=[img_size[1], img_size[0]]
        ).to(device)
    ...
    torch.save(
        {"feat": feat_bank[mask].half().cpu(), "mask_full": mask_entire},
...
def main(args):
    ...
    args.point2img_mapper = PointCloudToImageMapper(
        image_dim=img_dim, cut_bound=args.cut_num_pixel_boundary
    )
    ...
    if process_id_range is not None:
        id_range = [
            int(process_id_range[0].split(",")[0]),
            int(process_id_range[0].split(",")[1]),
        ]
