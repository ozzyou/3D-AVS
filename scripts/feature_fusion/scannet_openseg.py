...
def get_args():
    ...
    parser.add_argument(
        "--split", type=str, default="train", help='split: "train"| "val"'
    )
    parser.add_argument(
...
def process_one_scene(data_path, out_dir, args):
    ...
    if keep_features_in_memory and openseg_model is not None:
        ...
        for img_dir in tqdm(img_dirs):
            img_features.append(
                extract_openseg_img_feature(
                    img_dir, openseg_model, text_emb, img_size=[240, 320]
                )
            )
    ...
    for img_id, img_dir in enumerate(tqdm(img_dirs)):
        ...
        depth = (
            imageio.v2.imread(img_dir.replace("color", "depth").replace("jpg", "png"))
            / depth_scale
        )
