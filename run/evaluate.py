...
def evaluate(model, val_data_loader, labelset_name="scannet_3d"):
    ...
    text_features, labelset, mapper, palette = precompute_text_related_properties(
        labelset_name
    )
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label, feat_3d, mask, inds_reverse) in enumerate(
                ...
                if args.save_feature_as_numpy:
                    scene_name = (
                        val_data_loader.dataset.data_paths[i]
                        .split("/")[-1]
                        .split(".pth")[0]
                    )
                    np.save(
                ...
                if "nuscenes" in labelset_name:
                    ...
                    if vis_pred:
                        pcl = torch.load(val_data_loader.dataset.data_paths[i])[0][
                            label_mask
                        ]
                ...
                if vis_input:
                    ...
                    export_pointcloud(
                        os.path.join(save_folder, "{}_input.ply".format(i)),
                        pcl,
                ...
                if vis_pred:
                    if mapper is not None:
                        pred_label_color = convert_labels_with_palette(
                            mapper[logits_pred].numpy(), palette
                        )
                        export_pointcloud(
                            os.path.join(
                                save_folder, "{}_{}.ply".format(i, feature_type)
                            ),
                            pcl,
                        ...
                        visualize_labels(
                            ...
                            palette,
                            os.path.join(
                                save_folder, "{}_labels_{}.jpg".format(i, feature_type)
                            ),
                            ncol=5,
                        )
                ...
                if vis_gt:
                    ...
                    gt_label_color = convert_labels_with_palette(
                        label.cpu().numpy(), palette
                    )
                    export_pointcloud(
                        os.path.join(save_folder, "{}_gt.ply".format(i)),
                        pcl,
                    ...
                    visualize_labels(
                        ...
                        labelset,
                        palette,
                        os.path.join(save_folder, "{}_labels_gt.jpg".format(i)),
                        ncol=5,
                    )
                    ...
                    if "nuscenes" in labelset_name:
                        ...
                        visualize_labels(
                            ...
                            labelset,
                            palette,
                            os.path.join(save_folder, "{}_label.jpg".format(i)),
