...
def distill(train_loader, model, optimizer, epoch):
    ...
    export_pointcloud(
        os.path.join(
            ...
            "result",
            "last",
            "{}_{}.ply".format(args.feature_2d_extractor, epoch),
        ),
        pcl_part,
        colors=seg_label_color,
    )
    export_pointcloud(
        os.path.join(args.save_path, "result", "last", "pred_{}.ply".format(epoch)),
        pcl_part,
        colors=pred_label_color,
    )
    export_pointcloud(
        os.path.join(args.save_path, "result", "last", "gt_{}.ply".format(epoch)),
        pcl_part,
