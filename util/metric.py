...
def confusion_matrix(pred_ids, gt_ids, num_classes):
    ...
    return (
        np.bincount(
            pred_ids[idxs] * num_classes + gt_ids[idxs], minlength=num_classes**2
        )
...
def evaluate(pred_ids, gt_ids, stdout=False, dataset="scannet_3d"):
    ...
    for i in range(N_CLASSES):
        ...
        if (
            gt_ids == i
        ).sum() == 0:  # at least 1 point needs to be in the evaluation for this class
            continue
        ...
        for i in range(N_CLASSES):
            ...
            try:
                if "matterport" in dataset:
                    print(
                        "{0:<14s}: {1:>5.3f}".format(label_name, class_accs[label_name])
                    )
