...
class LossMeters:
    def __init__(self, keys=["clip", "blip"]):
        self.meters = {key: AverageMeter() for key in keys}

    def update(self, losses, n=1):
        for key, value in losses.items():
            self.meters[key].update(value.item(), n)

    def get_dict_val(self, key_postfix=""):
        return {key + key_postfix: meter.val for key, meter in self.meters.items() if meter.count > 0}

    def get_dict_avg(self):
        return {key: meter.avg for key, meter in self.meters.items() if meter.count > 0}
